...
def bot_answer(call):
    ...
    if call.data == 'create':
        ...
        def get_note(message):
            ...
            cursor.execute("INSERT INTO notes (user_id, note_text, note_date) VALUES (" + user_id + ", '" + msg_text + "', '" + msg_date + "')")
        ...
        def get_month(message):
            ...
            try:
                cursor.execute("SELECT note_text, note_date FROM notes WHERE user_id = " + user_id + " AND TO_CHAR(note_date, 'YYYY-MM') LIKE '" + msg_month + "'")
                month_answer = ''
        ...
        def get_day(message):
            ...
            cursor.execute("SELECT note_text, note_date FROM notes WHERE user_id = " + user_id + " AND TO_CHAR(note_date, 'YYYY-MM-DD') LIKE '" + msg_day + "'")
            day_answer = msg_day + '\n\n'
            day_get = False
